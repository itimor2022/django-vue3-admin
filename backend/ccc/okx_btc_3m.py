...
def get_btc():
    ...
    if n > 0.11:
        if return_0 > 0:
            msg = f'🈯 {title}<strike>单线涨幅超一个点</strike> <i>涨跌幅</i>:{return_now} 当前价:{close}'
        else:
            msg = f'🛑 {title}<strike>单线跌幅超一个点</strike> <i>涨跌幅</i>:{return_now} 当前价:{close}'
    ...
    if volume_x > 6.86:
        if return_0 > 0:
            msg = f'💹 {title}<strike>此时成交量超7倍</strike> {volume_x} <i>涨跌幅</i>:{return_now} 当前价:{close}'
        else:
            msg = f'💢 {title}<strike>此时成交量超7倍</strike> {volume_x} <i>涨跌幅</i>:{return_now} 当前价:{close}'
        send_message(msg, chat_id=chat_id)
